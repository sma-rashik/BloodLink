                  ...
                  <Popup>
                    <div className="text-center w-40">
                      ...
                      <div className="flex gap-2">
                         <a href={`tel:+${String(donor.phone || '').replace(/\D/g, '').startsWith('880') ? String(donor.phone || '').replace(/\D/g, '') : `880${String(donor.phone || '').replace(/\D/g, '').replace(/^0+/, '')}`}`} className="flex-1 text-center bg-gray-100 text-gray-700 font-bold rounded-md py-1.5 px-2 text-[10px] hover:bg-gray-200 transition">Call</a>
                         <a 
                           href={`https://wa.me/${String(donor.phone || '').replace(/\D/g, '').startsWith('880') ? String(donor.phone || '').replace(/\D/g, '') : `880${String(donor.phone || '').replace(/\D/g, '').replace(/^0+/, '')}`}?text=Hi%20${encodeURIComponent(donor.name)},%20I%20found%20you%20on%20ReDrop.%20I%20have%20an%20urgent%20need%20for%20${encodeURIComponent(donor.group)}%20blood.`}
